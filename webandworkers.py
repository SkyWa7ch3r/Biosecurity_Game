...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
		...
		parser.add_argument(
			'--runworkers', action='store',
			dest='runworkers', default=None,
			help='--runworkers how many runworkers you wish to run')
    ...
    def get_honcho_manager(self, options):
		...
		if options['runworkers'] != None:
			runworkers = option['runworkers']

        manager.add_process('daphne', daphne_cmd, env=self.get_env(options))
        for i in range(runworkers):
            manager.add_process(
                'worker{}'.format(i),
                'otree runworker',
